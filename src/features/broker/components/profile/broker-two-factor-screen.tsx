...
import { BrokerMfaOtpInput } from "./broker-mfa-otp-input";
import { BrokerMfaStepper } from "./broker-mfa-stepper";
...
export function BrokerTwoFactorScreen() {
  const t = useTranslations("broker.profile.twoFactor");
  const tc = useTranslations("common");
  const { data: session } = useSession();
  const userId = session?.user?.id;
  const accountLabel = session?.user?.email ?? "Broker";
  ...
  const totp = useMemo(() => {
    ...
    try {
      return createBrokerTotp(totpSecret, accountLabel);
    } catch {
      return null;
    }
