...
import type { AcceptAdminInvitationRequestDto } from "@/shared/api/dtos/iam";
...
export function AdminAcceptInvitePanel() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      const body: AcceptAdminInvitationRequestDto = {
        token,
        password,
        newPassword: password,
        confirmPassword,
      };
      await browserApi.post("/v1/auth/admin/invitations/accept", body);
      setSuccess(true);
    } catch (err) {
      ...
      <div className="flex flex-1 flex-col justify-center px-6 py-10 md:px-12 lg:px-16">
        <div className={cn(cardWrapClass, "text-center")}>
          ...
          <h2 className="font-heading text-foreground mt-5 text-xl font-bold tracking-tight">
            {t("acceptInviteMissingTokenTitle")}
          </h2>
          <p className="text-muted-foreground mt-2 text-sm leading-relaxed">
            {t("acceptInviteMissingTokenBody")}
          </p>
  ...
  if (success) {
    return (
      <div className="flex flex-1 flex-col justify-center px-6 py-10 md:px-12 lg:px-16">
        <div
          className="mx-auto flex w-full max-w-xl flex-col gap-8 rounded-xl border border-green-200 bg-green-50 px-5 py-4 text-green-900 dark:border-green-900/50 dark:bg-green-950/40 dark:text-green-100"
          role="status"
        >
          <div className="flex gap-3">
            ...
            <div className="min-w-0 space-y-1">
              <p className="font-heading text-base font-semibold text-green-900 dark:text-green-100">
                {t("acceptInviteSuccessTitle")}
              </p>
              <p className="text-sm leading-relaxed opacity-95">
                {t("acceptInviteSuccessBody")}
              </p>
            </div>
          </div>
          <Link
            href="/login"
            className="text-green-700 dark:text-green-300 ml-8 inline-flex items-center gap-1 text-sm font-semibold underline-offset-4 hover:underline"
          >
  ...
  return (
    <div className="flex flex-1 flex-col justify-center px-6 py-10 md:px-12 lg:px-16">
      <div className={cardWrapClass}>
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-6" noValidate>
          <div className="space-y-2">
            <Label htmlFor="accept-invite-password" className={labelClass}>
              {t("acceptInvitePasswordLabel")}
            </Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="accept-invite-confirm" className={labelClass}>
              {t("acceptInviteConfirmLabel")}
            </Label>
            <div className="relative">
              <Input
          ...
          <p className="text-muted-foreground flex gap-2 text-sm leading-relaxed">
            <Info
              ...
              aria-hidden
            />
            <span>{t("acceptInviteHelper")}</span>
          </p>
      ...
      {showCombinedError ? (
        <div
          className="border-destructive/30 bg-destructive/10 text-destructive mx-auto mt-6 w-full max-w-xl rounded-lg border px-4 py-3 text-sm"
          role="alert"
          aria-live="polite"
        >
          <div className="flex gap-3">
            ...
            <div className="min-w-0 space-y-2">
              <p className="font-semibold">{t("acceptInviteErrorTitle")}</p>
